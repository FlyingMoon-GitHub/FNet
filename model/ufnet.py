...
class UFNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.assist2 = self._assistant_layers(in_channels=256, first_kernel_size=7, loops=4)
        self.assist3 = self._assistant_layers(in_channels=512, first_kernel_size=5, loops=3)
        self.assist4 = self._assistant_layers(in_channels=1024, first_kernel_size=3, loops=2)
        self.assist5 = self._assistant_layers(in_channels=2048, first_kernel_size=1, loops=1)
    ...
    def forward(self, x_aux, x_prim):
        ...
        assist2 = self.assist2(c_aux_2)
        # print('assist_2', assist2.shape)
        c_prim_2 = c_prim_2 + assist2 * c_prim_2

        c_prim_3 = self.conv3_x_prim(c_prim_2)
        # print('conv3_prim', c_prim_3.shape)

        assist3 = self.assist3(c_aux_3)
        # print('assist_3', assist3.shape)
        c_prim_3 = c_prim_3 + assist3 * c_prim_3

        c_prim_4 = self.conv4_x_prim(c_prim_3)
        # print('conv4_prim', c_prim_4.shape)

        assist4 = self.assist4(c_aux_4)
        # print('assist_4', assist4.shape)
        c_prim_4 = c_prim_4 + assist4 * c_prim_4

        c_prim_5 = self.conv5_x_prim(c_prim_4)
        # print('conv5_prim', c_prim_5.shape)

        assist5 = self.assist5(c_aux_5)
        # print('assist_5', assist5.shape)
        c_prim_5 = c_prim_5 + assist5 * c_prim_5
    ...
    def saveGraph(self):
        with SummaryWriter(comment='ufnet', log_dir=self.log_dir) as sumw:
